...
/**
  ...
  * @param  uar pointer to a UART_HandleTypeDef structure that contains
...
void uart_init(uint8_t uar)
{
	...
	switch (uar){
		...
		case 1: {
			...
			GPIOA->MODER &= ~ (3<<20);   
			GPIOA->MODER |= 2<<20;
			GPIOA->AFR[1] |= 0x1<<4;  // alternate function code for uart
			GPIOA->AFR[1] |= 0x1<<8;
			USART1->BRR = 0x681;  // baud rate
			USART1->CR1 |= (1<<3) | (1<<0);  // transmitter  and peripheral enable
